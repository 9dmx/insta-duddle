...
def locate_robust(image_list):
    ...
    for image_file in image_list:
        ...
        if not os.path.isabs(image_file):
            image_file = os.path.join(os.path.dirname(__file__), image_file.lstrip('./'))
...
if __name__ == "__main__":
    ...
    title_label = ttk.Label(header_frame, text="flash.draw", font=("Helvetica", 24, "bold"))
    title_label.pack()
    subtitle_label = ttk.Label(header_frame, text="made with ❤ by flash", font=("Helvetica", 9), foreground="#aaaaaa")
    subtitle_label.pack()
    ...
    btn_calibrate = ttk.Button(ctrl_frame, text="1. Calibrate Canvas", command=gui_start_calibration, bootstyle="secondary-outline", width=30)
    btn_calibrate.pack(fill=BOTH, expand=True, padx=5, pady=5)

    btn_draw = ttk.Button(ctrl_frame, text="2. Start Drawing", command=gui_start_drawing, bootstyle="success-outline", width=30)
    ...
    status_var.set("✓ Ready. Select an image and calibrate.")
    lbl_status = ttk.Label(status_frame, textvariable=status_var, relief=SUNKEN, padding=8, font=("Helvetica", 9))
